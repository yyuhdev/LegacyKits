...
public class ConsumablesPage extends InventoryBuilder {

    public ConsumablesPage(Player player, boolean b){
        super(54, ColorUtil.of("<player>'s Kits"
                .replace("<player>", player.getName())
        ));
        KitRoomCache.getKitRoomPage(KitroomPage.CONSUMABLES).thenAccept(map -> {
        ...
        addCloseHandler(event -> Bukkit.getScheduler().runTaskLater(LegacyKits.getInstance(), () -> {
            ...
            if(b){
                ...
                KitRoomCache.saveKitRoomPage(KitroomPage.CONSUMABLES, content);
            }
        }, 1));
    }
}
