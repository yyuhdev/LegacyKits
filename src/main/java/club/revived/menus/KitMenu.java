...
public class KitMenu {
    ...
    public void init() {
        ...
        this.menu.button(43, Button.button(
                        ItemBuilder.item(Material.NETHERITE_HELMET).name(TextStyle.style("<#FFD1A3>Premade Kit")).lore(List.of(TextStyle.style("<white>Click to open"))))
                .onClick(event -> {
        ...
        for (int x = 19; x < 26; x++) {
            ...
            this.menu.button(37, Button.button(ItemBuilder.item(Material.BOOK)
                            .name(TextStyle.style("<#FFD1A3>Itemlist"))
                            .lore(TextStyle.style("<white>Click to open"))
            ...
            this.menu.button(40, Button.button(ItemBuilder.item(Material.NETHER_STAR)
                    .name(TextStyle.style("<#FFD1A3>Info"))
                    .lore(TextStyle.style("<white>Click to open the guide"))
            ).onClick(event -> {
                event.setCancelled(true);
                new GuideMenu(player).open();
            }));

            this.menu.button(52, Button.button(ItemBuilder.item(Material.CRAFTING_TABLE)
                    .name(TextStyle.style("<#FFD1A3>Crafting Menus"))
                    .lore(TextStyle.style("<white>Click to open"))
            ).onClick(event -> {
                event.setCancelled(true);
                new CraftingMenu(player).open();
            }));
        }
    }
